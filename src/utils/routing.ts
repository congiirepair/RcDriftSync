export type AppRoute =
  | { name: "home" | "garage" | "cars" | "tunes" | "builder" | "community" | "library" | "tips" | "sessions" | "admin" | "settings" | "login" | "signup" | "profile" | "notFound" }
  | { name: "adminCatalog" }
  | { name: "pdfMapper" }
  | { name: "tune"; tuneId: string }
  | { name: "share"; shareId: string }
  | { name: "track"; trackSlug: string }
  | { name: "brand"; brandSlug: string }
  | { name: "driverProfile"; username: string };

const staticRoutes = ["home", "garage", "cars", "tunes", "builder", "community", "library", "tips", "sessions", "admin", "settings", "login", "signup", "profile"] as const;
...
export function parseRoute(pathname = window.location.pathname): AppRoute {
  ...
  if (parts[0] === "admin" && parts[1] === "catalog") return { name: "adminCatalog" };
  ...
  if (parts[0] === "brands" && parts[1]) return { name: "brand", brandSlug: parts[1] };
